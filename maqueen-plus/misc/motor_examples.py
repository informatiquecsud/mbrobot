...
def motor(dirL, pL, dirR, pR):
    i2c.write(i2c_addr, bytearray([ALL_MOTORS, dirL, pL, dirR, pR]))
...
def rotate_both(power, target_pos, direction=DIR_FORWARD):
    reset_motor_pos()
    delay(100)
    
    # compenser le dépassement dû à l'inertie
    target_pos -= min(power / 3, 45)
    
    while True:
        degL, degR = read_motor_pos()
        if (degL + degR) / 2 > target_pos:
            break
        
        motor(direction, power, direction, power)
        delay(5)
...
def forward_cm(power, cm):
    target = cm2pos(cm)
    degL, degR = rotate_both(power, target)
    distance = (degL + degR) / 2
    print("distance [cm]: ", pos2cm(distance))
    rotate_both(50, (distance - target), DIR_BACKWARD)
